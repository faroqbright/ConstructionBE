...
async function getUserLanguage(userId) {
  ...
  try {
    ...
    console.warn(`Error fetching language preference for user ${userId}, defaulting to portuguese:`, error.message);
    return 'portuguese';
  }
}
...
async function getFinanceNotificationRecipients(project, performingUserIdStr) {
    ...
    if (project.members && Array.isArray(project.members)) {
        project.members.forEach(member => {
            if (member && member._id && !recipientUserObjectIds.has(member._id.toString())) {
                const token = member.notificationToken || member.fcmDeviceToken;
                recipientUserObjects.push({ ...(member.toObject ? member.toObject() : member), effectiveToken: token });
    ...
    if (project.projectOwners && Array.isArray(project.projectOwners)) {
        project.projectOwners.forEach(ownerObj => {
            if (ownerObj && ownerObj.ownerId && ownerObj.ownerId._id && !recipientUserObjectIds.has(ownerObj.ownerId._id.toString())) {
                const token = ownerObj.ownerId.notificationToken || ownerObj.ownerId.fcmDeviceToken;
                recipientUserObjects.push({ ...(ownerObj.ownerId.toObject ? ownerObj.ownerId.toObject() : ownerObj.ownerId), effectiveToken: token });
    ...
    if (performingUserIdStr && !recipientUserObjectIds.has(performingUserIdStr.toString())) {
        try {
            ...
            console.error(`Error fetching performing user ${performingUserIdStr}:`, error.message);
        }
    }
...
async function sendLocalizedPushNotifications(recipients, titleObj, bodyObj, data) {
    ...
    try {
        ...
        if (recipientsByLanguage.portuguese.length > 0) {
            const portugueseTokens = [...new Set(recipientsByLanguage.portuguese.map(u => u.effectiveToken).filter(Boolean))];
            if (portugueseTokens.length > 0) {
                sendPromises.push(sendPushNotificationFirebase(portugueseTokens, titleObj.portuguese, bodyObj.portuguese, data));
            }
        }
        if (recipientsByLanguage.english.length > 0) {
            const englishTokens = [...new Set(recipientsByLanguage.english.map(u => u.effectiveToken).filter(Boolean))];
            if (englishTokens.length > 0) {
                sendPromises.push(sendPushNotificationFirebase(englishTokens, titleObj.english, bodyObj.english, data));
            }
        }
        if (sendPromises.length > 0) {
            await Promise.all(sendPromises);
            console.log(`Localized push notifications initiated for: ${titleObj.english} / ${titleObj.portuguese}`);
        } else {
            console.warn("[sendLocalizedPushNotifications] No valid tokens found for any language group.");
        }
    } catch (error) {
...
export const uploadFinanceDocument = async (req, res) => {
  ...
  const performingUserId = req.user?._id; // Get performing user ID as string
  ...
  try {
    ...
    const project = await editProject
      ...
      .populate("projectOwners.ownerId", "_id email userName notificationToken fcmDeviceToken") // Populate needed fields
      .populate("members", "_id email userName notificationToken fcmDeviceToken") // Populate needed fields
      .session(session)
      .lean(); // Use lean for performance if not modifying project doc directly for save
    ...
    const recipients = await getFinanceNotificationRecipients(project, performingUserId ? performingUserId.toString() : null);
    ...
    await session.commitTransaction(); // Commit before sending external notifications (email, push)

    // 2. Push Notifications (after commit)
    ...
    // 3. Email Notifications (after commit)
    for (const recipient of recipients) {
        ...
        try {
            await SendEmailUtil({ from: process.env.EMAIL_USER, to: recipient.email, subject, html });
            console.log(`Email sent to ${recipient.email} (Lang: ${userLanguage})`);
        } catch (error) {
...
export const getFinanceDocuments = async (req, res) => {
  try {
    ...
    }).select("projectName").lean(); // Only need projectName
    ...
    const query = (isMain && projectNames.length === 0) ? {} : { projName: { $in: projectNames } };
    const financeDocuments = await FinanceDocument.find(query).sort({ uploadedAt: -1 }).lean();
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(req.body, "financialExecution")) {
      ...
      if (isNaN(finExec) || finExec < 0 || finExec > 100) {
        ...
        return res.status(400).json({ message: "Financial Execution must be a number between 0 and 100" });
      }
      if (existingDocument.financialExecution !== finExec) {
        newFinancialExecution = finExec; changesMade = true;
        changeDescriptions.portuguese.push(`Execução Financeira alterada para ${finExec}%`);
        changeDescriptions.english.push(`Financial Execution changed to ${finExec}%`);
      }
    }
    ...
    if (Object.prototype.hasOwnProperty.call(req.body, "physicalExecution")) {
      ...
      if (isNaN(phyExec) || phyExec < 0 || phyExec > 100) {
        await session.abortTransaction(); session.endSession();
        return res.status(400).json({ message: "Physical Execution must be a number between 0 and 100" });
      }
      if (existingDocument.physicalExecution !== phyExec) {
        newPhysicalExecution = phyExec; changesMade = true;
        changeDescriptions.portuguese.push(`Execução Física alterada para ${phyExec}%`);
        changeDescriptions.english.push(`Physical Execution changed to ${phyExec}%`);
      }
    }
    ...
    if (req.body.fileName && typeof req.body.fileName === "string" && req.body.fileName.trim() !== "" && existingDocument.fileName !== req.body.fileName.trim()) {
        updates.fileName = req.body.fileName.trim(); changesMade = true;
        changeDescriptions.portuguese.push(`Nome do arquivo alterado para "${updates.fileName}"`);
        changeDescriptions.english.push(`Filename changed to "${updates.fileName}"`);
    }
    if (req.body.reference && typeof req.body.reference === "string" && existingDocument.reference !== req.body.reference) {
        updates.reference = req.body.reference; changesMade = true;
        changeDescriptions.portuguese.push(`Referência alterada para "${updates.reference}"`);
        changeDescriptions.english.push(`Reference changed to "${updates.reference}"`);
    }
    ...
    if (changesMade) {
      ...
      await session.abortTransaction(); session.endSession();
      return res.status(200).json({ message: "No substantive changes detected. Document not updated.", document: existingDocument });
    }

    const updatedFinanceDocument = await FinanceDocument.findByIdAndUpdate(
      id, { $set: updates }, { new: true, runValidators: true, session }
    );
    ...
    if (!project) {
        console.warn(`Project "${existingDocument.projName}" not found for notifications during update.`);
    ...
    for (const recipient of recipients) {
        const userLanguage = await getUserLanguage(recipient._id);
        const isPortuguese = userLanguage === 'portuguese';
        const changesStringPt = changeDescriptions.portuguese.join(', ') || "detalhes atualizados";
        const changesStringEn = changeDescriptions.english.join(', ') || "details updated";
        ...
        inAppNotifications.push({
            title, type: "Document Update", description, lengthyDesc,
            memberId: recipient._id, projectId: project?._id
        });
    }
    if (inAppNotifications.length > 0) {
        await ShowNotification.create(inAppNotifications, { session });
    ...
    await sendLocalizedPushNotifications(
        ...
        {
            type: "Finance Document Updated", projectId: project?._id.toString() || "",
            documentId: updatedFinanceDocument._id.toString(), documentName: updatedFinanceDocument.fileName
        }
    );
    ...
    for (const recipient of recipients) {
        ...
        const userLanguage = await getUserLanguage(recipient._id);
        const isPortuguese = userLanguage === 'portuguese';
        const changesString = isPortuguese ? (changeDescriptions.portuguese.join('; ') || "detalhes foram atualizados") : (changeDescriptions.english.join('; ') || "details were updated");
        ...
        try {
            await SendEmailUtil({ from: process.env.EMAIL_USER, to: recipient.email, subject, html });
        } catch (e) { console.error(`Email error to ${recipient.email}:`, e.message); }
    }
    // --- END LOCALIZED NOTIFICATIONS ---

    res.status(200).json({ message: "Document updated, notifications sent.", document: updatedFinanceDocument });
  } catch (error) {
    if (session.inTransaction()) await session.abortTransaction();
    console.error("Error updating document:", error);
    res.status(error.statusCode || 500).json({ message: error.message || "Error updating document" });
  } finally {
    session.endSession();
  }
};
  ...
  try {
    ...
    if (financeDocument.fileUrl) {
      try {
        ...
        else console.warn(`Could not extract file key from URL: ${financeDocument.fileUrl}`);
      } catch (s3Error) { console.error("Error deleting file from S3:", s3Error.message); }
    }
    ...
    // 1. In-app Notifications
    const inAppNotifications = [];
    if (project) { // Only send if project context is clear
        for (const recipient of recipients) {
            const userLanguage = await getUserLanguage(recipient._id);
            const isPortuguese = userLanguage === 'portuguese';
            ...
            inAppNotifications.push({
                title, type: "Document Deletion", description, lengthyDesc,
                memberId: recipient._id, projectId: project._id
            });
        }
        if (inAppNotifications.length > 0) {
    ...
    if (project) { // Only send if project context is clear
        await sendLocalizedPushNotifications(
            ...
            {
                type: "Finance Document Deleted", projectId: project._id.toString(),
                documentName: financeDocument.fileName, deletedDocumentId: financeDocument._id.toString()
            }
        );
    }
