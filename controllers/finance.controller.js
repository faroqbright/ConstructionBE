...
export const updateFinanceDocument = async (req, res) => {
  const session = await mongoose.startSession(); // Use session for update
  session.startTransaction();
  ...
  try {
    ...
    if (req.body.reference) updates.reference = req.body.reference;

    // Check if only timestamp would be updated (if no other valid fields are present in req.body)
    const updateKeys = Object.keys(updates);
    const reqBodyKeys = Object.keys(req.body);
    const relevantUpdateProvided = reqBodyKeys.some(key => ['financialExecution', 'physicalExecution', 'fileName', 'reference'].includes(key));

    if (updateKeys.length === 1 && !relevantUpdateProvided) { // Only uploadedAt and no other relevant changes
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({ message: "No substantive changes detected to update." });
    }

    const updatedFinanceDocument = await FinanceDocument.findByIdAndUpdate(id, { $set: updates }, { new: true, session });
    if (!updatedFinanceDocument) { // Should not happen if findById worked
        await session.abortTransaction(); session.endSession();
    ...
    const inAppNotificationData = {
      ...
      memberId: performingUserId, // Notifier is the one performing action
      projectId: project?._id,
    };
    ...
    // Push Notification Recipients (owners, members of the project, and performing user)
    const pushNotificationRecipientIds = [];
    if (project) {
        project.projectOwners?.forEach(o => { if(o.ownerId?._id) pushNotificationRecipientIds.push(o.ownerId._id); });
        project.members?.forEach(m => { if(m?._id) pushNotificationRecipientIds.push(m._id); });
    }
    if (performingUserId) pushNotificationRecipientIds.push(performingUserId);
    ...
    if (allEmails.length > 0) {
      const emailBody = {
        ...
        text: `Hello,\n\nA finance document "${updatedFinanceDocument.fileName}" for the project "${project?.projectName || existingDocument.projName}" has been updated by ${req.user?.userName || 'System'}.\n\nBest regards,\nYour Team`,
      };
      try {
        await SendEmailUtil(emailBody);
