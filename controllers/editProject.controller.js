...
async function sendProjectNotificationEmail(
  ...
  try {
    ...
    const userLanguage = languagePref?.languageSelected || "portuguese";
    const safePerformerName = performingUser?.userName || "System";
    const safeProjectName = projectInfo.projectName || "The Project";

    const templates = {
      update: {
        portuguese: `<!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8"><title>Atualização de Projeto</title></head><body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;"><table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px; margin: auto; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"><tr><td style="padding: 20px; text-align: left;"><h2 style="color: #333;">Notificação de Atualização de Projeto</h2><p style="font-size: 16px; color: #555;">O projeto <strong>${safeProjectName}</strong> foi atualizado por ${safePerformerName}.</p><p style="font-size: 16px; color: #555;">Resumo das alterações:</p><ul style="font-size: 16px; color: #555; padding-left: 20px;">${changesSummary.map((change) => `<li>${change}</li>`).join("")}</ul><p style="font-size: 16px; color: #555;">Por favor, faça login para ver os detalhes completos.</p><p style="font-size: 14px; color: #999; margin-top: 30px;">Esta é uma notificação automática.</p></td></tr></table></body></html>`,
        english: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Project Update Notification</title></head><body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;"><table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px; margin: auto; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"><tr><td style="padding: 20px; text-align: left;"><h2 style="color: #333;">Project Update Notification</h2><p style="font-size: 16px; color: #555;">The project <strong>${safeProjectName}</strong> has been updated by ${safePerformerName}.</p><p style="font-size: 16px; color: #555;">Summary of changes:</p><ul style="font-size: 16px; color: #555; padding-left: 20px;">${changesSummary.map((change) => `<li>${change}</li>`).join("")}</ul><p style="font-size: 16px; color: #555;">Please log in to view the complete details.</p><p style="font-size: 14px; color: #999; margin-top: 30px;">This is an automated notification.</p></td></tr></table></body></html>`,
      },
      deletion: {
        portuguese: `<!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8"><title>Projeto Eliminado</title></head><body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;"><table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px; margin: auto; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"><tr><td style="padding: 20px; text-align: left;"><h2 style="color: #333;">Notificação de Eliminação de Projeto</h2><p style="font-size: 16px; color: #555;">O projeto <strong>${safeProjectName}</strong> foi eliminado por ${safePerformerName}.</p><p style="font-size: 16px; color: #555;">Todos os documentos e dados associados a este projeto foram removidos do sistema.</p><p style="font-size: 14px; color: #999; margin-top: 30px;">Esta é uma notificação automática.</p></td></tr></table></body></html>`,
        english: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Project Deleted</title></head><body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;"><table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px; margin: auto; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"><tr><td style="padding: 20px; text-align: left;"><h2 style="color: #333;">Project Deletion Notification</h2><p style="font-size: 16px; color: #555;">The project <strong>${safeProjectName}</strong> has been deleted by ${safePerformerName}.</p><p style="font-size: 16px; color: #555;">All documents and data associated with this project have been removed from the system.</p><p style="font-size: 14px; color: #999; margin-top: 30px;">This is an automated notification.</p></td></tr></table></body></html>`,
      },
      creation: {
        portuguese: `<!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8"><title>Novo Projeto Criado</title></head><body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;"><table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px; margin: auto; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"><tr><td style="padding: 20px; text-align: left;"><h2 style="color: #333;">Notificação de Novo Projeto</h2><p style="font-size: 16px; color: #555;">Um novo projeto <strong>${safeProjectName}</strong> foi criado por ${safePerformerName}.</p><p style="font-size: 16px; color: #555;">Por favor, faça login para ver os detalhes do projeto.</p><p style="font-size: 14px; color: #999; margin-top: 30px;">Esta é uma notificação automática.</p></td></tr></table></body></html>`,
        english: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>New Project Created</title></head><body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;"><table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; max-width: 600px; margin: auto; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);"><tr><td style="padding: 20px; text-align: left;"><h2 style="color: #333;">New Project Notification</h2><p style="font-size: 16px; color: #555;">A new project <strong>${safeProjectName}</strong> has been created by ${safePerformerName}.</p><p style="font-size: 16px; color: #555;">Please log in to view the project details.</p><p style="font-size: 14px; color: #999; margin-top: 30px;">This is an automated notification.</p></td></tr></table></body></html>`,
      },
    };
    await SendEmailUtil({
      from: process.env.EMAIL_FROM || "noreply@example.com",
      ...
      subject:
        userLanguage === "portuguese"
          ? type === "update"
            ...
            : type === "deletion"
              ? `Projecto Eliminado: ${safeProjectName}`
              : `Novo Projecto: ${safeProjectName}`
          : type === "update"
            ? `Project Update: ${safeProjectName}`
            : type === "deletion"
              ? `Project Deleted: ${safeProjectName}`
              : `New Project: ${safeProjectName}`,
      html: templates[type][userLanguage],
    });
  } catch (emailError) {
    console.error(
...
async function sendLanguageSpecificPushNotifications(users, title, body, data) {
  try {
    ...
    users.forEach((user) => {
      const token = user.fcmDeviceToken || user.notificationToken;
      if (token) {
        const language = userLanguageMap[user._id.toString()] || "portuguese";
        tokensByLanguage[language].push(token);
      }
    });
    ...
    if (tokensByLanguage.portuguese.length > 0) {
      await sendPushNotification(
        ...
        console.error("Error sending Portuguese push notification:", e.message)
      );
    }
    if (tokensByLanguage.english.length > 0) {
      await sendPushNotification(
        ...
        console.error("Error sending English push notification:", e.message)
      );
    }
  } catch (error) {
    console.error("Error sending language-specific push notifications:", error);
...
const editProjects = asyncHandler(async (req, res) => {
  ...
  let updatedProject; // Declare here for broader scope
  ...
  try {
    ...
    const {
      ...
      location: newLocationInput,
      businessAreas: newBusinessAreasInput,
      companyName: newCompanyNameInput, // Corrected from 'comapanyName' if that was a typo
      members: newMemberIdsInput,
      status: newStatusInput,
      deadline: newDeadlineInput,
      physicalEducationRange: newPhysicalEducationRangeInput,
    ...
    const logs = [];
    let updatedProjectBanners = [...(existingProject.projectBanner || [])];
    const changesSummary = []; // This will be mutated for the 'isFirstUpdate' case
    let importantFieldsChanged = false;
    let membersListChanged = false;
    let ownersListChanged = false;
    let statusChangedToCompleted = false;
    ...
    if (Array.isArray(newMemberIdsInput)) {
      ...
      const existingMemberIds = (existingProject.members || [])
        .map((m) => (m?._id?.toString ? m._id.toString() : null))
        .filter(Boolean);
    ...
    if (Array.isArray(newProjectOwnerIdsInput)) {
      ...
      const existingOwnerIds = (existingProject.projectOwners || [])
        .map((o) =>
          o?.ownerId?._id?.toString ? o.ownerId._id.toString() : null
        )
        .filter(Boolean);
    ...
    if (Object.keys(updateData).length === 0) {
      // No direct field updates
      // If only isCreated changed, updateData would be { isCreated: false }
      // So this check is fine. If logs were added without field changes, it would be caught by $push logic.
      return res
        .status(200)
    ...
    try {
      ...
      // Determine if a general notification should be triggered
      ...
      if (isFirstUpdate) {
        ...
        if (changesSummary.length === 0) {
          // Provide a default summary for the notification message if none exists from other changes.
          changesSummary.push("Project has been activated."); // This mutates the original changesSummary
        }
      }

      if (
        shouldTriggerGeneralNotification &&
        ...
        // This uses the potentially modified changesSummary
        const safeChangesText = changesSummary.map(String).join("; ");

        userIdsToQuery.forEach((userIdStr) => {
          const userLanguage = userLanguageMap[userIdStr] || "portuguese";
          const notificationType = isFirstUpdate
            ? "Project Creation"
            : "Project Update";
      ...
      if (statusChangedToCompleted) {
        ...
        // reviewUserIds.delete(performingUserIdStr); // Optional: Don't ask performer to review their own completion action
      ...
      if (!finalProjectDataForNotif) {
        ...
        try {
          // The `changesSummary` array used here (as `finalChangesSummary`) will have the "Project has been activated."
          // message if it was an `isFirstUpdate` without other specific changes.
          ...
          notificationRecipientsMap.delete(performingUser._id.toString()); // Don't send push/email to performer
          ...
          if (usersToNotify.length > 0) {
            ...
            // Push Notifications (General Updates / Creation)
            if (
              (isFirstUpdate ||
            ...
            // Email Notifications (General Updates / Creation)
            if (
              (isFirstUpdate ||
                importantFieldsChanged ||
                membersListChanged ||
            ...
            // Push Notifications (Review Request)
            if (statusChangedToCompleted) {
              const reviewUserIds = new Set();
...
const getAllProjects = asyncHandler(async (req, res) => {
  ...
  const { isMain, _id: loggedInUserId, assignedBusinessAreas = [] } = req.user;
  const validStatuses = ["Ongoing", "Pending", "Completed", "Awaiting Start", "On Hold", "Cancelled", "Archived"];

  let baseFilter = {};
  if (status && validStatuses.includes(status)) baseFilter.status = status;
  if (search) {
    const searchRegex = new RegExp(search, "i");
    baseFilter.$or = [
      { projectName: searchRegex }, { description: searchRegex },
      { location: searchRegex }, { companyName: searchRegex }, // Consistent field name
    ];
  }

  let finalFilter = { ...baseFilter };
  if (!isMain) {
    const userBusinessAreas = assignedBusinessAreas.map(area => area.businessArea).filter(Boolean);
    const userAccessConditions = [
      { members: loggedInUserId }, { "projectOwners.ownerId": loggedInUserId },
    ];
    if (userBusinessAreas.length > 0) {
      userAccessConditions.push({ businessAreas: { $in: userBusinessAreas } });
    }

    if (userAccessConditions.length > 0) {
      if (Object.keys(finalFilter).length > 0 && finalFilter.$or && !finalFilter.$and) { // If baseFilter had $or (from search)
         finalFilter = { $and: [ { $or: finalFilter.$or } , { $or: userAccessConditions }] };
      } else if (Object.keys(finalFilter).length > 0) {
         finalFilter = { $and: [finalFilter, { $or: userAccessConditions }] };
      }
      else {
  ...
  let query = editProject.find(finalFilter)
    .populate([{ path: "members", select: "userName avatar role", populate: { path: "role", select: "roleName" } },
               { path: "projectOwners.ownerId", select: "userName role", populate: { path: "role", select: "roleName" } }])
    .sort({ createdAt: -1 });
  ...
  if (milestoneUserIds) {
    try {
      ...
      if (Array.isArray(parsedMilestoneUserIds) && parsedMilestoneUserIds.length > 0) {
        const validUserIds = parsedMilestoneUserIds.filter(id => mongoose.Types.ObjectId.isValid(id));
        if (validUserIds.length > 0) {
          const projectIds = projects.map(p => p._id);
          const milestonesData = await AdditionalMilestone.find({ projectId: { $in: projectIds }, userId: { $in: validUserIds } }).select("projectId").lean();
          const projectsWithMilestones = new Set(milestonesData.map(m => m.projectId.toString()));
          filteredProjectsByMilestone = projects.filter(p => projectsWithMilestones.has(p._id.toString()));
        }
      }
    } catch (e) { console.warn("Error parsing milestoneUserIds:", e.message); }
  }

  const projectIds = filteredProjectsByMilestone.map(p => p._id);
  const projectNames = filteredProjectsByMilestone.map(p => p.projectName).filter(Boolean);
  let allUserDocs = [], allSystemDocs = [], allFinanceDocs = [], allAdditionalMilestones = [];

  if (projectNames.length > 0) {
    [allUserDocs, allSystemDocs, allFinanceDocs] = await Promise.all([
      UserDocument.find({ projName: { $in: projectNames } }).sort({ uploadedAt: -1 }).lean(),
      Document.find({ projName: { $in: projectNames } }).sort({ uploadedAt: -1 }).lean(),
  ...
  if (projectIds.length > 0) {
    allAdditionalMilestones = await AdditionalMilestone.find({ projectId: { $in: projectIds } })
      .populate({ path: "userId", model: "User", select: "userName" }).sort({ createdAt: -1 }).lean();
  }

  const groupDocsBy = (docs, keyField) => docs.reduce((acc, doc) => {
    const key = doc[keyField]?.toString();
    if (key) (acc[key] = acc[key] || []).push(doc);
    return acc;
  }, {});
  ...
  const projectsWithDetails = filteredProjectsByMilestone.map(project => {
    const isMember = project.members?.some(m => m._id.equals(loggedInUserId)) || false;
    const isOwner = project.projectOwners?.some(o => o.ownerId?._id.equals(loggedInUserId)) || false;
    const projectBAs = Array.isArray(project.businessAreas) ? project.businessAreas : (project.businessAreas ? [project.businessAreas] : []);
    const userBAs = assignedBusinessAreas.map(area => area.businessArea).filter(Boolean);
    const fromBusinessArea = !isMember && !isOwner && projectBAs.some(area => userBAs.includes(area));
    return {
      ...project,
      accessType: { isMember, isOwner, fromBusinessArea },
      documents: (userDocsMap[project.projectName] || []).map(d => ({ ...d, id: d._id, uploadedAt: d.uploadedAt || d.createdAt })),
      financeDocuments: (financeDocsMap[project.projectName] || []).map(d => ({ ...d, id: d._id })),
      projectReports: (systemDocsMap[project.projectName] || []).map(r => ({ ...r, id: r._id, uploadedAt: r.uploadedAt || r.createdAt })),
      additionalMilestones: additionalMilestonesMap[project._id.toString()] || [],
    };
  });

  res.status(200).json(new ApiResponse(200, {
    projects: projectsWithDetails,
    pagination: { currentPage: pageNumber, totalPages: Math.ceil(totalProjects / pageSize), totalProjects },
...
const getProjectById = asyncHandler(async (req, res) => {
  ...
  const project = await editProject.findById(projectId)
    .populate([{ path: "members", model: "User", select: "userName avatar role userType", populate: { path: "role", model: "Role", select: "roleName" } },
               { path: "projectOwners.ownerId", model: "User", select: "userName role email", populate: { path: "role", select: "roleName" } }])
    .lean();
  ...
  if (!isMain) {
    ...
    const isOwner = project.projectOwners?.some(o => o.ownerId?._id.equals(loggedInUserId)) || false;
    const projectBAs = Array.isArray(project.businessAreas) ? project.businessAreas : (project.businessAreas ? [project.businessAreas] : []);
    const userBAs = assignedBusinessAreas.map(ba => ba.businessArea).filter(Boolean);
  ...
  const [projectUserDocs, projectSystemDocs, projectFinanceDocs, projectAdditionalMilestones] = await Promise.all([
    UserDocument.find({ projName: project.projectName }).sort({ uploadedAt: -1 }).lean(),
    Document.find({ projName: project.projectName }).sort({ uploadedAt: -1 }).lean(),
    FinanceDocument.find({ projName: project.projectName }).sort({ uploadedAt: -1 }).lean(),
    AdditionalMilestone.find({ projectId: project._id }).populate({ path: "userId", model: "User", select: "userName" }).sort({ createdAt: -1 }).lean(),
  ]);

  const currentMilestones = [
    { name: "Project Details", completed: true, key: "details" }, { name: "Filling", completed: false, key: "filling" },
    { name: "Payment", completed: false, key: "payment" }, { name: "Review", completed: false, key: "review" },
    { name: "Completed", completed: false, key: "project_completed" },
  ];
  if (project.description && project.location && project.projectName && project.projectBanner?.length > 0 && project.members?.length > 0) {
    (currentMilestones.find(m => m.key === "filling") || {}).completed = true;
  }
  if (projectFinanceDocs.length > 0) (currentMilestones.find(m => m.key === "payment") || {}).completed = true;
  if ((currentMilestones.find(m => m.key === "filling")?.completed) && (currentMilestones.find(m => m.key === "payment")?.completed)) {
    (currentMilestones.find(m => m.key === "review") || {}).completed = true;
  }
  if (project.status === "Completed") (currentMilestones.find(m => m.key === "project_completed") || {}).completed = true;

  const responseData = {
    ...project,
    members: project.members?.map(m => ({ userId: m._id, _id: m._id, userName: m.userName, avatar: m.avatar, userType: m.userType || "N/A", role: m.role?.roleName || "N/A" })) || [],
    projectOwners: project.projectOwners?.filter(o => o.ownerId).map(o => ({ ownerId: o.ownerId._id, ownerName: o.ownerId.userName || "", role: o.ownerId.role?.roleName || "N/A", _id: o.ownerId._id, email: o.ownerId.email || "" })) || [],
    documents: projectUserDocs.map(d => ({ ...d, id: d._id, uploadedAt: d.uploadedAt || d.createdAt })),
    financeDocuments: projectFinanceDocs.map(d => ({ ...d, id: d._id })),
    projectReports: projectSystemDocs.map(r => ({ ...r, id: r._id, uploadedAt: r.uploadedAt || r.createdAt })),
    latestLog: project.logs?.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))[0] || null,
...
//   if (!mongoose.Types.ObjectId.isValid(projectId)) throw new ApiError(400, "Invalid Project ID");
//   if (!performingUser || !performingUser._id) throw new ApiError(401, "Unauthorized action");
// import mongoose from "mongoose";
// import { v4 as uuidv4 } from "uuid";
// import { editProject } from "../models/project.model.js";
// import { User } from "../models/user.model.js";
...
const editProjects = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // --- Check if any updates were made ---
    if (Object.keys(updateData).length === 0) { // No direct field updates
      // If only isCreated changed, updateData would be { isCreated: false }
      // So this check is fine. If logs were added without field changes, it would be caught by $push logic.
    ...
    try {
      ...
      if (inAppNotificationsToCreate.length > 0) {
        try {
          ...
          console.error(`In-app notification error (project ${projectId}):`, notificationError); // Log and continue
        }
      }
...
const deleteProject = asyncHandler(async (req, res) => {
  ...
  session.startTransaction();
  try {
    const projectToDelete = await editProject.findById(projectId)
      .populate("projectOwners.ownerId", "email userName _id notificationToken fcmDeviceToken")
      .populate("members", "email userName _id notificationToken fcmDeviceToken")
      .session(session).lean();
    if (!projectToDelete) throw new ApiError(404, "Project not found.");

    const deletedProjectName = projectToDelete.projectName;
    const deletedProjectId = projectToDelete._id;

    await editProject.findByIdAndDelete(projectId, { session });
    await UserDocument.deleteMany({ projName: deletedProjectName }, { session });
    await Document.deleteMany({ projName: deletedProjectName }, { session });
    await FinanceDocument.deleteMany({ projName: deletedProjectName }, { session });
    await AdditionalMilestone.deleteMany({ projectId: deletedProjectId }, { session });
    
    // Notifications
    const recipientsMap = new Map();
    (projectToDelete.members || []).forEach(u => u?._id && recipientsMap.set(u._id.toString(), u));
    (projectToDelete.projectOwners || []).forEach(o => o?.ownerId?._id && recipientsMap.set(o.ownerId._id.toString(), o.ownerId));
    // OPTION: Include performing user in delete notifications
    let performerForNotif = recipientsMap.get(performingUser._id.toString());
    if (!performerForNotif) {
        performerForNotif = await User.findById(performingUser._id).select("email userName _id notificationToken fcmDeviceToken").lean();
        if (performerForNotif) recipientsMap.set(performingUser._id.toString(), performerForNotif);
    } else {
       recipientsMap.set(performingUser._id.toString(), { ...performerForNotif, ...performingUser });
    }
    // To EXCLUDE performing user: recipientsMap.delete(performingUser._id.toString());
    
    const usersToNotify = Array.from(recipientsMap.values());

    if (usersToNotify.length > 0) {
      const langMap = await getUserLanguagePreferences(usersToNotify.map(u => u._id.toString()));
      const inAppNotifs = usersToNotify.map(user => {
        const lang = langMap[user._id.toString()] || "portuguese";
        return {
          title: lang === "portuguese" ? `Projeto Eliminado: ${deletedProjectName}` : `Project Deleted: ${deletedProjectName}`,
          type: "Project Deletion",
          description: lang === "portuguese" ? `O projeto "${deletedProjectName}" foi eliminado por ${performingUser.userName}.` : `The project "${deletedProjectName}" was deleted by ${performingUser.userName}.`,
          lengthyDesc: lang === "portuguese" ? `Todos os dados do projeto "${deletedProjectName}" foram removidos. Ação por ${performingUser.userName}.` : `All data for project "${deletedProjectName}" has been removed. Action by ${performingUser.userName}.`,
          memberId: user._id, projectId: deletedProjectId, // projectId might be less useful here as it's deleted
        };
      });
      if (inAppNotifs.length > 0) await ShowNotification.insertMany(inAppNotifs, { session, ordered: false });
    }
    await session.commitTransaction(); // Commit before sending non-transactional notifications

    // Post-transaction notifications
    if (usersToNotify.length > 0) {
        await sendLanguageSpecificPushNotifications(
            usersToNotify.filter(u => u.fcmDeviceToken || u.notificationToken),
            { portuguese: `Projeto Eliminado: ${deletedProjectName}`, english: `Project Deleted: ${deletedProjectName}` },
            { portuguese: `O projeto "${deletedProjectName}" foi eliminado por ${performingUser.userName}.`, english: `The project "${deletedProjectName}" was deleted by ${performingUser.userName}.` },
            { deletedProjectId: deletedProjectId.toString(), type: "PROJECT_DELETED" }
        );
        for (const user of usersToNotify.filter(u => u.email)) {
            await sendProjectNotificationEmail(user, { projectName: deletedProjectName }, performingUser, [], "deletion");
        }
    }
    res.status(200).json(new ApiResponse(200, { deletedProjectId, projectName: deletedProjectName }, "Project deleted successfully."));
  } catch (error) {
    await session.abortTransaction();
    console.error("Error deleting project:", error);
    const statusCode = error instanceof ApiError ? error.statusCode : 500;
    const message = error instanceof ApiError ? error.message : "Error deleting project.";
    res.status(statusCode).json(new ApiResponse(statusCode, null, message, error.errors || []));
  } finally {
    session.endSession();
  }
});
