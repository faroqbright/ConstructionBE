...
export const createOrUpdateBusinessArea = async (req, res) => {
  const { businessArea, email, projectName } = req.body;

  try {
    if (!businessArea || !email || !projectName) {
...
export const getAllBusinessAreas = async (req, res) => {
  try {
    const businessAreas = await BusinessArea.find({}, 'businessArea email projectName createdAt')
      .sort({ createdAt: -1 });
...
export const getSingleBusinessArea = async (req, res) => {
  ...
  try {
    const businessArea = await BusinessArea.findById(id, 'businessArea email projectName createdAt');
...
export const updateBusinessArea = async (req, res) => {
  ...
  try {
    ...
    const updated = await BusinessArea.findByIdAndUpdate(
      id,
      { businessArea, email, projectName },
      { new: true, fields: 'businessArea email projectName createdAt' }
    );
