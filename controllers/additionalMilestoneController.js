      ...
      const emailPromises = allRecipients.map(async user => {
        if (user?.email) {
          ...
          try {
            ...
            console.error(`Error sending update email to ${user.email}:`, err.message);
          }
        }
      });
    ...
    const creationEmailPromises = allRecipients.map(async user => {
      if (user?.email) {
        ...
        try {
          ...
          console.error(`Error sending creation email to ${user.email}:`, err.message);
        }
      }
    });
...
export const updateMilestone = async (req, res) => {
  ...
  try {
    ...
    if (updates.description) existingMilestone.description = updates.description;
    if (updates.status) existingMilestone.status = updates.status;
    if (updates.completedAt) existingMilestone.completedAt = updates.completedAt;
    ...
    if (updates.title && updates.title !== originalTitle) {
      changes.push(`<p><strong>Title Updated:</strong> "${originalTitle}" → "${updates.title}"</p>`);
    }
    if (updates.description && updates.description !== originalDescription) {
      changes.push(`<p><strong>Description Updated:</strong> ${originalDescription || "N/A"} → ${updates.description}</p>`);
    }
    if (updates.status && updates.status !== originalStatus) {
      changes.push(`<p><strong>Status Updated:</strong> ${originalStatus} → ${updates.status}</p>`);
    }

    const changeMessage = changes.length > 0 ? changes.join("") : "<p>No specific fields changed.</p>";
